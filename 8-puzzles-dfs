...
// mengecek bahwa nilai yang telah diinputkan telah masuk ke matriksnya
bool test() {
  int cnt1 = 0, cnt2 = 0;
...
//Mengambil matrix saat ini untuk diperiksa apakah sudah mencapai goal state
...
//mengecek apakah path selanjutnya pernah dikunjungi apa blm
bool visited(Node* a) {
  Node* par = a->par;
  while(par != NULL) {
...
//menambahkan path sebagai child
...
int main() {
  ...
  Node* first = new Node(); //membuat node pertama
  first->par = NULL;
  first->move = Right; // hanya untuk langkah awal
  first->nn = 0;
